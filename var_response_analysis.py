...
def loadvar(direc,name,num,model_params):
    
    out = np.array([[resp(x) for x in np.loadtxt(direc + "_VAR_RUN_{}/".format(k) + name)] for k in range(1,num+1)])

    #[num,LEN,4]
    #[num*LEN,4]

    if len(name.split("clean")) == 1:
        nt = model_params["ntrial"]
    else:
        nt = 1

    nres = out.shape[-1]
    
    out = np.reshape(out,[num,5,nt,out.shape[1]/(5*nt),nres])
    out = np.transpose(out,[1,3,4,0,2])
    out = np.reshape(out,[5,out.shape[1],nres,num*nt])
    out = np.transpose(out,[0,3,1,2])
    out = np.reshape(out,[-1,nres])

    return out

def MGSM_resp(DATFILE):

    data = DATFILE.split("_VAR")[0]    

    with open(DATFILE + "model_params.json", 'r') as fp:
    ...
    size_tuning_noisy = loadvar(data,"size_tuning_noisy.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "size_tuning_noisy.csv")])
    size_tuning_clean = loadvar(data,"size_tuning_clean.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "size_tuning_clean.csv")])

    size_tuning_noisy = np.reshape(size_tuning_noisy,[5,10*model_params["ntrial"],len(con),size_tuning_noisy.shape[0]/(5*len(con)*ntrial*10),4])
    size_tuning_clean = np.reshape(size_tuning_clean,[5,10,len(con),size_tuning_clean.shape[0]/(5*len(con)*10),4])
    ...
    full_field_noisy = loadvar(data,"full_field_noisy.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "full_field_noisy.csv")])
    full_field_clean = loadvar(data,"full_field_clean.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "full_field_clean.csv")])
    
    full_field_noisy = np.reshape(full_field_noisy,[5,10*model_params["ntrial"],len(con),4])
    full_field_clean = np.reshape(full_field_clean,[5,10,len(con),4])
...
#import the surround surppression data
    ...
    surr_supp_noisy = loadvar(data,"surr_supp_noisy.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "surr_supp_noisy.csv")])
    surr_supp_clean = loadvar(data,"surr_supp_clean.csv",10,model_params)
    #np.array([resp(x) for x in np.loadtxt(data + "surr_supp_clean.csv")])
        
    surr_supp_noisy = np.reshape(surr_supp_noisy,[5,10*model_params["ntrial"],5,len(con),2,4])
    surr_supp_clean = np.reshape(surr_supp_clean,[5,10,5,len(con),2,4])
...
#    surr_supp_SP_clean = np.reshape(surr_supp_SP_clean,[5,1,5,len(con),2,5]).mean(axis = 1)
    ...
    #COS data
    if COSplot:
        FF_COS_noisy = loadvar(data,"full_field_COS_noisy.csv",10,model_params)
        #np.array([resp(x) for x in np.loadtxt(data + "full_field_COS_noisy.csv")])
        FF_COS_clean = loadvar(data,"full_field_COS_clean.csv",10,model_params)
        #np.array([resp(x) for x in np.loadtxt(data + "full_field_COS_clean.csv")])
        
        FF_COS_noisy = np.reshape(FF_COS_noisy,[5,10*model_params["ntrial"],n_cos_a,len(con),len(con),4])
        FF_COS_clean = np.reshape(FF_COS_clean,[5,10,n_cos_a,len(con),len(con),4])
    ...
    if WTAplot:
        FF_WTA_noisy = loadvar(data,"full_field_WTA_noisy.csv",10,model_params)
        #np.array([resp(x) for x in np.loadtxt(data + "full_field_WTA_noisy.csv")])
        FF_WTA_clean = loadvar(data,"full_field_WTA_clean.csv",10,model_params)
        #np.array([resp(x) for x in np.loadtxt(data + "full_field_WTA_clean.csv")])
        
        FF_WTA_noisy = np.reshape(FF_WTA_noisy,[5,10*model_params["ntrial"],len(con),32,2,4])
        FF_WTA_clean = np.reshape(FF_WTA_clean,[5,10,len(con),32,2,4])
...
#make FF figures
    ...
    for k in range(len(FF_n_mean)):
        ...
        plt.xscale("log")
        plt.savefig(data+"FF_plot_{}.pdf".format(k))
...
#plot of all FF responses
        ...
        plt.savefig(data+"FF_plot_{}.pdf".format(len(FF_n_mean)))
...
#SS Shape: [k,d_ang,con,condition,f_ang]
    ...
    for k in range(len(SS_n_mean)):
        for d in range(len(SS_n_mean[k])):
            ...
            plt.savefig(data+"SS_plot_{}_{}.pdf".format(k,d))
...
#            plt.savefig(data+"SS_SP_plot_{}_{}.pdf".format(k,d))
    ...
    for k in range(len(ST_n_mean)):
        ...
        plt.savefig(data+"all_ST_plot_{}.pdf".format(k))        
        ...
        plt.savefig(data+"RFsize_plot_{}.pdf".format(k))
...
#        plt.set_xrange([0,1.5*np.max(ST_n_mean[k])])
        ...
        plt.savefig(data+"ST_plot_{}.pdf".format(k))
        ...
        plt.savefig(data+"ST_SI_{}.pdf".format(k))
        ...
        # plt.savefig(data+"ST_SP_plot_{}.pdf".format(k))
...
#COS shape: [k,d_ang,tar_con,mas_con,f_ang]
        for k in range(len(FFC_n_mean)):
            for d in range(len(FFC_n_mean[k])):
                ...
                plt.savefig(data+"COS_plot_{}_{}.pdf".format(k,d))
        ...
        for k in range(len(FFC_n_mean)):
            for d in range(len(FFC_n_mean[k])):
                ...
                plt.savefig(data+"COS_CRF_{}_{}.pdf".format(k,d))
        ...
        if WTAplot:
            for k in range(len(FFW_n_mean)):
                ...
                for c1 in range(len(ind)):
                    ...
                    plt.savefig(data+"COS_WTA_{}.pdf".format(k))
        ...
        for k in range(len(FFC_n_mean)):
            for d in range(len(FFC_n_mean[k])):
                ...
                plt.savefig(data+"COS_CON_{}_{}.pdf".format(k,d))
                ...
                plt.savefig(data+"nnCOS_CON_{}_{}.pdf".format(k,d))
...
def GSM_resp(DATFILE):
    ...
    for k in range(len(FFW_n_mean)):
        ...
        plt.savefig(data+"ORI_TUN_{}.pdf".format(k))
...
#                     ,"k--")
    ...
    for k in range(len(FFC_n_mean)):
        for d in [-1]:
            ...
            plt.savefig(data+"COS_CON_{}.pdf".format(k))
            ...
            plt.savefig(data+"nnCOS_CON_{}.pdf".format(k))
            ...
            plt.savefig(data+"COS_CON_zoomed_{}.pdf".format(k))
    ...
    for k in range(len(FFC_n_mean)):
        for d in [-1]:
            ...
            plt.savefig(data+"COS_CRF_{}.pdf".format(k))
    ...
    for k in range(len(FFC_n_mean)):
        for d in range(len(FFC_n_mean[k])):
            ...
            plt.savefig(data+"COS_AI_{}_{}.pdf".format(k,d))
