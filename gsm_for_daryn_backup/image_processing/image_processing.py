...
def get_phased_filter_coefficients(pic, na, npha, k, s, t, meansub = False,lap = True):
    if lap:
        if npha != 2:
            print("warning! when using DOG ffilters npha = 2, not {}".format(npha))

        kernels = np.array([[LAPc(aa,k,t),LAPs(aa,k,t)] for aa in [i * math.pi / na for i in range(na)]])
    else:
...
def get_filter_maps(nf, na, npha, k, s, t, d, lap = True):
    if lap:
        if npha != 2:
            print("warning! when using DOG ffilters npha = 2, not {}".format(npha))
...
def get_phased_filter_samples(iname,nf,na,npha,k,s,t,d,samd,imsize = 500,mode = "file",MS = False):
    ...
    fil = get_phased_filter_coefficients(img,na,npha,k,s,t,meansub = MS)
